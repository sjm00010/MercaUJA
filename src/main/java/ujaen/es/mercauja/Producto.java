...
public class Producto {
    ...
    /**
     * Establece el precio actual para el producto en función de la puja más
     * alta
     *
     * @param comprador Comprador que realiza la puja
     * @param precio Nuevo precio del producto
     */
    public void pujar(Comprador comprador, int precio) {
        lock.lock();  // Bloquea mientras se edita el precio
        try {
            if (this.comprador != null) {
                if (!comprador.getName().equals(this.comprador.getName())
                        && comprador.getDinero() >= precio && precio > precioActual) {
                    this.comprador.devolverDeposito(precioActual);
                    precioActual = precio;
                    this.comprador = comprador;
                    this.comprador.realizarDeposito(precioActual);
                }
    ...
    public String toString() {
        return "PRODUCTO DEL " + getVendedor().getName() + " : " + precioActual + " € | " + (vendido ? "VENDIDO" : "CANCELADO") + (vendido ? " a " + comprador.getName() : "");
    }
}
